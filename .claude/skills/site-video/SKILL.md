...
- **Видео-отзыв клиента** → `components/TestimonialsSection.tsx`, ассеты в
  ...
  рядом с именем (`md:hidden`). В каждый момент в DOM живёт ровно ОДИН
  `<GifVideo>` — какой именно, решает `isMobile` через matchMedia, во втором
  слоте статичный постер (см. раздел «КРИТИЧНО» ниже).
...
## Если видео «не крутится» на iPhone — порядок диагностики

1. Посчитай `<video>` с этим src в DOM: больше одного (видимый + скрытый
   дубликат) — вот причина, см. раздел «КРИТИЧНО».
2. Проверь SSR-разметку (`curl … | grep -o '<video[^>]*>'`): должны быть
   `muted`, `playsInline`, `autoPlay`. Отсутствие muted в SSR лечится
   страховкой в GifVideo (mount-эффект).
3. Спроси про режим энергосбережения на телефоне: он глушит автозапуск
   ЛЮБЫХ видео — это не баг, покажется постер, запуск по тапу (наш клик
   по видео и так вызывает play()).
4. Помни: headless-Chrome с мобильным вьюпортом НЕ воспроизводит политику
   автозапуска iOS Safari — «в тесте играет» ничего не гарантирует для
   айфона, финальная проверка только на реальном телефоне после деплоя.
